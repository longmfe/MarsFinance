...
def get_hist_data_by_daterange(
    code,
    hist_start_date,
    hist_end_date,
    fields=["open", "close", "high", "low", "volume", "preClose"],
...
def _order(code, amount):
    ...
    if context.positions.get(code, 0) < -amount:
        ...
        print(
            "can not sell security exceding the account holding, adjusted to %d"
            % amount
        )
...
def run():
    ...
    bm_df = get_hist_data_by_daterange(
        context.benchmark, context.bkt_start_date, context.bkt_end_date
    )
