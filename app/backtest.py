...
class PortfolioBacktest:
    ...
    def run_stock_universe_backtest(
        self,
        stock_data_dict,
        strategy_function,
        start_date=None,
        ...
        for stock_code, data in stock_data_dict.items():
            ...
            if backtest.portfolio_values:
                initial_value = (
                    backtest.portfolio_values[0] if backtest.portfolio_values else 1
                )
                normalized_values = [
                    v / initial_value for v in backtest.portfolio_values
                ]
    ...
    def get_portfolio_metrics(self):
        ...
        total_final_value = sum(
            result["final_value"] for result in self.stock_results.values()
        )
        total_initial_value = sum(
            result["initial_capital"] for result in self.stock_results.values()
        )
        portfolio_return = (
            total_final_value - total_initial_value
        ) / total_initial_value

        # 平均指标
        avg_annual_return = np.mean(
            [r["annual_return"] for r in self.stock_results.values()]
        )
        ...
        # 正收益股票比例
        positive_returns = len(
            [r for r in self.stock_results.values() if r["total_return"] > 0]
        )
    ...
    def print_detailed_report(self):
        ...
        if self.stock_results:
            ...
            for stock, metrics in sorted_stocks[:5]:
                print(
                    f"  {stock}: {metrics['total_return']:.2%} (交易次数: {metrics['total_trades']})"
                )
    ...
    def plot_portfolio_performance(self):
        ...
        # 1. 所有股票的归一化收益曲线
        plt.subplot(2, 2, 1)
        for stock_code, data in list(self.portfolio_values.items())[
            :20
        ]:  # 只显示前20只股票
            if len(data["dates"]) > 0:
        ...
        plt.axvline(
            ...
            color="red",
            linestyle="--",
            label=f"AvgReturn: {np.mean(returns):.2%}",
        )
        ...
        plt.axvline(
            ...
            color="red",
            linestyle="--",
            label=f"AvgSharpe: {np.mean(sharpes):.2f}",
        )
        ...
        plt.axvline(
            ...
            color="red",
            linestyle="--",
            label=f"avg: {np.mean(drawdowns):.2%}",
        )
...
def main():
    ...
    stock_data_dict = generate_hs300_sample_data(
        stock_codes, start_date="20240101", end_date="20241231"
    )
