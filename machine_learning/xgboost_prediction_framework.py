...
def create_features(df):
    ...
    df["Price_Position"] = (df["Close"] - df["Low_52w"]) / (
    ...
    df["Positive_MF"] = np.where(
        df["Close"] > df["Close"].shift(1), df["Raw_Money_Flow"], 0
    )
    # 负向资金流（下跌日）
...
def prepare_data(df, test_size=0.2):
    ...
    feature_columns = [
        ...
        if col
        not in ["Target_30d", "Open", "High", "Low", "Close", "Adj_Close", "Volume"]
    ]
...
def visualize_predictions(y_true, y_pred, dates=None, title="预测结果对比"):
    ...
    confusion_df = pd.DataFrame(
        confusion_data, index=["实际上涨", "实际下跌"], columns=["预测上涨", "预测下跌"]
    )
    ...
    # 计算精确率、召回率
    precision_up = (
        correct_up / (correct_up + wrong_up) if (correct_up + wrong_up) > 0 else 0
    )
    recall_up = (
        correct_up / (correct_up + wrong_down) if (correct_up + wrong_down) > 0 else 0
    )
    precision_down = (
        correct_down / (correct_down + wrong_down)
        if (correct_down + wrong_down) > 0
        else 0
    )
    recall_down = (
        correct_down / (correct_down + wrong_up) if (correct_down + wrong_up) > 0 else 0
    )
