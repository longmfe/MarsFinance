...
目标变量：未来 30 日收益率（回归任务）。特征四族：动量（多周期收益、52 周价格
位置、动量加速度）、量能（量比、简化 MFI 资金流、量价背离）、技术指标（均线相对
位置、RSI、MACD、布林带）、波动与情绪（多周期波动率、连涨连跌天数）。

纪律：按时间顺序切分训练/测试（不 shuffle），StandardScaler 仅在训练集拟合，
